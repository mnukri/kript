...
import type { Charge, ChargeType, ChargeStatus } from '@/lib/types'
...
const chargeTypeColor: Record<string, string> = {
  effort:   'bg-blue-50 text-blue-700',
  purchase: 'bg-zinc-100 text-zinc-600',
...
export default function ManageCharges() {
  ...
  const sorted = [...charges].sort((a, b) => b.charge_date.localeCompare(a.charge_date))

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-500">{charges.length} charge records</p>
            ...
            {sorted.map((c) => (
              <tr key={c.charge_id} className="border-b border-zinc-100">
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${chargeTypeColor[c.charge_type]}`}>{c.charge_type}</span>
                </td>
      ...
      {open && (
        <Modal title={editing ? 'Edit Charge' : 'Add Charge'} onClose={() => setOpen(false)}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Charge Type" required>
                <select className={selectCls} value={form.charge_type} onChange={(e) => set('charge_type', e.target.value as ChargeType)}>
                  <option value="effort">Effort (Salary)</option>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Amount ($)">
                <input className={inputCls} type="number" step="0.01" value={form.amount ?? ''} onChange={(e) => set('amount', e.target.value === '' ? null : Number(e.target.value))} />
              </Field>
            </div>
